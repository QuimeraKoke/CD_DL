...
# Parámetros
max_length = 256 # Aumentamos un poco la longitud para pares de frases
batch_size = 16

# Crear una función para tokenizar los pares de frases
def tokenize_sentence_pairs(example):
    ...
    return {'input_ids': inputs['input_ids'], 'attention_mask': inputs['attention_mask']}, example['label']

# Aplicar la tokenización al dataset
ds_train_tokenized = ds_train.map(tokenize_sentence_pairs)
ds_validation_tokenized = ds_validation.map(tokenize_sentence_pairs)
